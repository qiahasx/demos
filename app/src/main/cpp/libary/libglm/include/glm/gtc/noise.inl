...
{
detail::tvec3 <T> pXYZ =
        floor(fract(detail::tvec3<T>(j) * detail::tvec3<T>(ip)) * T(7)) * ip[2] - T(1);
...
{
detail::tvec4 <T> Pi =
        glm::floor(detail::tvec4<T>(P.x, P.y, P.x, P.y)) + detail::tvec4<T>(0.0, 0.0, 1.0, 1.0);
detail::tvec4 <T> Pf =
        glm::fract(detail::tvec4<T>(P.x, P.y, P.x, P.y)) - detail::tvec4<T>(0.0, 0.0, 1.0, 1.0);
...
detail::tvec4 <T> norm = taylorInvSqrt(
        detail::tvec4<T>(dot(g00, g00), dot(g01, g01), dot(g10, g10), dot(g11, g11)));
g00 *= norm.
x;
g01 *= norm.
y;
...
detail::tvec4 <T> n_z = mix(detail::tvec4<T>(n000, n100, n010, n110),
                            detail::tvec4<T>(n001, n101, n011, n111), fade_xyz.z);
detail::tvec2 <T> n_yz = mix(detail::tvec2<T>(n_z.x, n_z.y), detail::tvec2<T>(n_z.z, n_z.w),
                             fade_xyz.y);
...
detail::tvec4 <T> norm00 = taylorInvSqrt(
        detail::tvec4<T>(dot(g0000, g0000), dot(g0100, g0100), dot(g1000, g1000),
                         dot(g1100, g1100)));
g0000 *= norm00.
x;
g0100 *= norm00.
...
detail::tvec4 <T> norm01 = taylorInvSqrt(
        detail::tvec4<T>(dot(g0001, g0001), dot(g0101, g0101), dot(g1001, g1001),
                         dot(g1101, g1101)));
g0001 *= norm01.
x;
g0101 *= norm01.
...
detail::tvec4 <T> norm10 = taylorInvSqrt(
        detail::tvec4<T>(dot(g0010, g0010), dot(g0110, g0110), dot(g1010, g1010),
                         dot(g1110, g1110)));
g0010 *= norm10.
x;
g0110 *= norm10.
...
detail::tvec4 <T> norm11 = taylorInvSqrt(
        detail::tvec4<T>(dot(g0011, g0011), dot(g0111, g0111), dot(g1011, g1011),
                         dot(g1111, g1111)));
g0011 *= norm11.
x;
g0111 *= norm11.
...
detail::tvec4 <T> n_0w = mix(detail::tvec4<T>(n0000, n1000, n0100, n1100),
                             detail::tvec4<T>(n0001, n1001, n0101, n1101), fade_xyzw.w);
detail::tvec4 <T> n_1w = mix(detail::tvec4<T>(n0010, n1010, n0110, n1110),
                             detail::tvec4<T>(n0011, n1011, n0111, n1111), fade_xyzw.w);
detail::tvec4 <T> n_zw = mix(n_0w, n_1w, fade_xyzw.z);
detail::tvec2 <T> n_yzw = mix(detail::tvec2<T>(n_zw.x, n_zw.y), detail::tvec2<T>(n_zw.z, n_zw.w),
                              fade_xyzw.y);
...
{
detail::tvec4 <T> Pi =
        floor(detail::tvec4<T>(P.x, P.y, P.x, P.y)) + detail::tvec4<T>(0.0, 0.0, 1.0, 1.0);
detail::tvec4 <T> Pf =
        fract(detail::tvec4<T>(P.x, P.y, P.x, P.y)) - detail::tvec4<T>(0.0, 0.0, 1.0, 1.0);
...
// x2 = x0 - 1.0 + 2.0 * C.xx ;
detail::tvec4 <T> x12 =
        detail::tvec4<T>(x0.x, x0.y, x0.x, x0.y) + detail::tvec4<T>(C.x, C.x, C.z, C.z);
...
detail::tvec4 <T> a0 = detail::tvec4<T>(b0.x, b0.z, b0.y, b0.w) +
                       detail::tvec4<T>(s0.x, s0.z, s0.y, s0.w) *
                       detail::tvec4<T>(sh.x, sh.x, sh.y, sh.y);
detail::tvec4 <T> a1 = detail::tvec4<T>(b1.x, b1.z, b1.y, b1.w) +
                       detail::tvec4<T>(s1.x, s1.z, s1.y, s1.w) *
                       detail::tvec4<T>(sh.z, sh.z, sh.w, sh.w);
...
// Normalise gradients
detail::tvec4 <T> norm = taylorInvSqrt(
        detail::tvec4<T>(dot(p0, p0), dot(p1, p1), dot(p2, p2), dot(p3, p3)));
p0 *= norm.
x;
p1 *= norm.
y;
...
detail::tvec4 <T> m = max(
        T(0.6) - detail::tvec4<T>(dot(x0, x0), dot(x1, x1), dot(x2, x2), dot(x3, x3)), T(0));
m = m * m;
return T(42) *
dot(m
* m,
...
detail::tvec3 <T> isYZ = step(detail::tvec3<T>(x0.z, x0.w, x0.w),
                              detail::tvec3<T>(x0.y, x0.y, x0.z));
