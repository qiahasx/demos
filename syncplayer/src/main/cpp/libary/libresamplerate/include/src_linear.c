...
linear_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    if (!priv->dirty) {    /* If we have just been reset, set the last_value data. */
    ...
    while (input_index < 1.0 && priv->out_gen < priv->out_count) {
        ...
        if (priv->out_count > 0 && fabs(state->last_ratio - data->src_ratio) > SRC_MIN_RATIO_DIFF)
            src_ratio = state->last_ratio +
                        priv->out_gen * (data->src_ratio - state->last_ratio) / priv->out_count;

        for (ch = 0; ch < state->channels; ch++) {
            data->data_out[priv->out_gen] = (float) (priv->last_value[ch] + input_index *
                                                                            ((double) data->data_in[ch] -
                                                                             priv->last_value[ch]));
            priv->out_gen++;
        };
    ...
    while (priv->out_gen < priv->out_count &&
        ...
        for (ch = 0; ch < state->channels; ch++) {
            data->data_out[priv->out_gen] = (float) (
                    data->data_in[priv->in_used - state->channels + ch] + input_index *
                                                                          ((double) data->data_in[
                                                                                  priv->in_used +
                                                                                  ch] -
                                                                           data->data_in[
