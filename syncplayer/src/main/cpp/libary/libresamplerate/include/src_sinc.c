...
typedef int _CHECK_SHIFT_BITS[
        2 * (SHIFT_BITS < sizeof(increment_t) * 8 - 1) - 1]; /* sanity check. */
...
static SRC_ERROR prepare_data(SINC_FILTER *filter, int channels, SRC_DATA *data,
                              int half_filter_chan_len) WARN_UNUSED;
...
sinc_filter_new(int converter_type, int channels) {
    ...
    if (priv) {
        ...
        priv->b_len = 3 * (int) psf_lrint(
                (priv->coeff_half_len + 2.0) / priv->index_inc * SRC_MAX_RATIO + 1);
...
sinc_copy(SRC_STATE *state) {
    ...
    memcpy(to_filter->buffer, from_filter->buffer,
           sizeof(float) * (from_filter->b_len + state->channels));
...
calc_output_single(SINC_FILTER *filter, increment_t increment, increment_t start_filter_index) {
    ...
    while (filter_index >= MAKE_INCREMENT_T (0)) {
        ...
        icoeff =
                filter->coeffs[indx] + fraction * (filter->coeffs[indx + 1] - filter->coeffs[indx]);
...
sinc_mono_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    filter->b_current =
            (filter->b_current + state->channels * psf_lrint(input_index - rem)) % filter->b_len;
    input_index = rem;
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        if (samples_in_hand <= half_filter_chan_len) {
            if ((state->error = prepare_data(filter, state->channels, data,
        ...
        if (filter->out_count > 0 && fabs(state->last_ratio - data->src_ratio) > 1e-10)
            src_ratio = state->last_ratio +
                        filter->out_gen * (data->src_ratio - state->last_ratio) / filter->out_count;
        ...
        data->data_out[filter->out_gen] = (float) ((float_increment / filter->index_inc) *
                                                   calc_output_single(filter, increment,
                                                                      start_filter_index));
        filter->out_gen++;
        ...
        filter->b_current = (filter->b_current + state->channels * psf_lrint(input_index - rem)) %
                            filter->b_len;
        input_index = rem;
    };
...
static inline void
calc_output_stereo(SINC_FILTER *filter, int channels, increment_t increment,
...
sinc_stereo_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_stereo(filter, state->channels, increment, start_filter_index,
                           float_increment / filter->index_inc, data->data_out + filter->out_gen);
        filter->out_gen += 2;
...
static inline void
calc_output_quad(SINC_FILTER *filter, int channels, increment_t increment,
                 increment_t start_filter_index, double scale, float *output) {
...
sinc_quad_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_quad(filter, state->channels, increment, start_filter_index,
                         float_increment / filter->index_inc, data->data_out + filter->out_gen);
        filter->out_gen += 4;
...
static inline void
calc_output_hex(SINC_FILTER *filter, int channels, increment_t increment,
                increment_t start_filter_index, double scale, float *output) {
...
sinc_hex_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_hex(filter, state->channels, increment, start_filter_index,
                        float_increment / filter->index_inc, data->data_out + filter->out_gen);
        filter->out_gen += 6;
...
static inline void
calc_output_multi(SINC_FILTER *filter, increment_t increment, increment_t start_filter_index,
                  int channels, double scale, float *output) {
...
sinc_multichan_vari_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    while (filter->out_gen < filter->out_count) {
        ...
        calc_output_multi(filter, increment, start_filter_index, state->channels,
                          float_increment / filter->index_inc, data->data_out + filter->out_gen);
...
prepare_data(SINC_FILTER *filter, int channels, SRC_DATA *data, int half_filter_chan_len) {
    ...
    if (filter->b_current == 0) {    /* Initial state. Set up zeros at the start of the buffer and
		** then load new data after that.
		*/
        len = filter->b_len - 2 * half_filter_chan_len;

        filter->b_current = filter->b_end = half_filter_chan_len;
    } else if (filter->b_end + half_filter_chan_len + channels <
               filter->b_len) {    /*  Load data at current end position. */
        len = MAX (filter->b_len - filter->b_current - half_filter_chan_len, 0);
    } else {    /* Move data at end of buffer back to the start of the buffer. */
    ...
    if (filter->in_used == filter->in_count &&
        filter->b_end - filter->b_current < 2 * half_filter_chan_len && data->end_of_input) {    /* Handle the case where all data in the current buffer has been
		** consumed and this is the last buffer.
		*/

        if (filter->b_len - filter->b_end < half_filter_chan_len +
                                            5) {    /* If necessary, move data down to the start of the buffer. */
            len = filter->b_end - filter->b_current;
            memmove(filter->buffer, filter->buffer + filter->b_current - half_filter_chan_len,
                    (half_filter_chan_len + len) * sizeof(filter->buffer[0]));
