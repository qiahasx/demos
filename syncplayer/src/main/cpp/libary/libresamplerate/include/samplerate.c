...
src_process(SRC_STATE *state, SRC_DATA *data) {
    ...
    if (data->data_in < data->data_out) {
        if (data->data_in + data->input_frames * state->channels > data->data_out) {    /*-printf ("\n\ndata_in: %p    data_out: %p\n",
				(void*) (data->data_in + data->input_frames * psrc->channels), (void*) data->data_out) ;-*/
            return SRC_ERR_DATA_OVERLAP;
        };
    } else if (data->data_out + data->output_frames * state->channels > data->data_in) {    /*-printf ("\n\ndata_in : %p   ouput frames: %ld    data_out: %p\n", (void*) data->data_in, data->output_frames, (void*) data->data_out) ;
...
src_callback_read(SRC_STATE *state, double src_ratio, long frames, float *data) {
    ...
    output_frames_gen = 0;
    while (output_frames_gen < frames) {    /*	Use a dummy array for the case where the callback function
